...
## 2026-06-20 - Implement 'Skip to content' for SPA accessibility
**Learning:** In Single Page Applications (SPAs), a 'Skip to content' link must target a container with `tabIndex={-1}` and `outline-none` to ensure that programmatic focus is accepted without creating a visual ring on the entire content area. This is essential for keyboard users to bypass global navigation.
**Action:** When adding skip-links, ensure all page-level components provide a valid target container with the correct accessibility attributes.
