...
## 2026-05-18 - Implement global Skip to Content bypass link and consistent NotFound container focus
**Learning:** For single page applications (SPAs), a global skip-to-content bypass link is critical for keyboard accessibility. However, it requires every page-level container to consistently declare `id="main-content"`, a programmatic `tabIndex={-1}`, and `outline-none` so that focus moves cleanly to the main element without introducing unwanted visual focus outlines.
**Action:** Always map page-level layouts to the matching main bypass ID and ensure focus states are styled with premium glow rings instead of default browser outlines.
